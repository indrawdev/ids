...
class Jual extends CI_Controller
{
	...
	function cust_kode()
	{
		...
		$this->load->model('mSearch','',true);
		$ssql = $this->mSearch->cust_jual_all($kdcust,$nmcust,$cari);
		$total = $ssql->num_rows();
		
		$ssql = $this->mSearch->cust_jual($kdcust,$nmcust,$nstart,$cari);
	...
	function cust_ceksave()
	{
		...
		if (trim($nmcust) == '' or trim($kdid) == '' or trim($almt) == '' or trim($tlp) == '')
		{
			$hasil = array(
				'sukses'	=> false,
				'hasil'		=> 'One of the names, id card, address, or phone is still empty!!</br>Please fill in advance!!'
			);
		...
		else
		{
			...
			$ssql = $this->mCust->cek_idcard($kdid,$nmcust);
			
			if ($ssql->num_rows() > 0)
			{
				$hasil = array(
					'sukses'	=> true,
					'hasil'		=> 'Customer already exists, do you want to update it?'
				);
				echo json_encode($hasil);
			}
			else
	...
	function cust_save()
	{
		...
		if ($xupdate == false)
		{
			if (trim($kdcust) <> '' and trim($kdcount) <> '')
			{
				...
				$hasil = array(
					'sukses'	=> true,
					'hasil'		=> 'Saving Customer Success',
					'kdcust'	=> trim($kdcust),
					'kdcount'	=> trim($kdcount),
					'nmcust'	=> trim($nmcust),
			...
			else
			{
				$hasil = array(
					'sukses'	=> false,
					'hasil'		=> 'Saving Customer Failed, generate customer code failed!!</br>Please try again letter...'
				);
				echo json_encode($hasil);
			}
		}
		else
		{
			...
			$hasil = array(
				'sukses'	=> true,
				'hasil'		=> 'Saving Update Customer Success',
				'kdcust'	=> trim($kdcust),
				'kdcount'	=> trim($kdcount),
				'nmcust'	=> trim($nmcust),
	...
	function kodedp()
	{
		...
		$this->load->model('mSearch','',true);
		$ssql = $this->mSearch->dp_all($kdcode,$cari);
		$total = $ssql->num_rows();
		
		$ssql = $this->mSearch->dp($kdcode,$nstart,$cari);
	...
	function product()
	{
		...
		$this->load->model('mSearch','',true);
		$ssql = $this->mSearch->product_all($kdprod,$nmprod,$cari);
		$total = $ssql->num_rows();
		
		$ssql = $this->mSearch->product($kdprod,$nmprod,$nstart,$cari);
	...
	function payment()
	{
		...
		$this->load->model('mSearch','',true);
		$ssql = $this->mSearch->payment_all($kdpay,$nmpay,$cari);
		$total = $ssql->num_rows();
		
		$ssql = $this->mSearch->payment($kdpay,$nmpay,$nstart,$cari);
	...
	function refno()
	{
		...
		$this->load->model('mSearch','',true);
		$ssql = $this->mSearch->jual_unit_all($trx,$refno,$docno,$cust,'0','0',$cari);
		$total = $ssql->num_rows();
		
		$ssql = $this->mSearch->jual_unit($trx,$refno,$docno,$cust,'0','0',$nstart,$total,$cari);
	...
	function sales()
	{
		...
		$this->load->model('mSearch','',true);
		$ssql = $this->mSearch->sales_all($kdsales,$nmsales,$cari);
		$total = $ssql->num_rows();
		
		$ssql = $this->mSearch->sales($kdsales,$nmsales,$nstart,$cari);
	...
	function sales_mtd()
	{
		...
		$this->load->model('mSearch','',true);
		$ssql = $this->mSearch->sales_mtd_all($kdsalesmtd,$nmsalesmtd,$cari);
		$total = $ssql->num_rows();
		
		$ssql = $this->mSearch->sales_mtd($kdsalesmtd,$nmsalesmtd,$nstart,$cari);
	...
	function survey()
	{
		...
		$this->load->model('mSearch','',true);
		$ssql = $this->mSearch->survey_all($kdsurvey,$nmsurvey,$cari);
		$total = $ssql->num_rows();
		
		$ssql = $this->mSearch->survey($kdsurvey,$nmsurvey,$nstart,$cari);
	...
	function term()
	{
		...
		$this->load->model('mSearch','',true);
		$ssql = $this->mSearch->term_all($kdterm,$nmterm,$cari);
		$total = $ssql->num_rows();
		
		$ssql = $this->mSearch->term($kdterm,$nmterm,$nstart,$cari);
	...
	function rangka()
	{
		...
		$this->load->model('mSearch','',true);
		$ssql = $this->mSearch->rangka_jual_all($refno,$rangka,$mesin,$kdprod,$wh,$cari);
		$total = $ssql->num_rows();
		
		$ssql = $this->mSearch->rangka_jual($refno,$rangka,$mesin,$kdprod,$wh,$nstart,$cari);
	...
	function griddetil()
	{
		...
		$ssql = $this->mJual->jual_unit_all($trx,$refno,$docno,$cust,'0','0',$cari);
		$total = $ssql->num_rows();
		
		$ssql = $this->mJual->jual_unit($trx,$refno,$docno,$cust,'0','0',$nstart,$total,$cari);
	...
	function gridjurnal()
	{
		...
		if ($sSQL->num_rows() > 0)
		{
			...
			{
				$xArr[] = array(
						'fs_acno' => ascii_to_entities(trim($xRow->fs_acno)),
						'fn_debit' => ascii_to_entities(trim(number_format($xRow->d, 2))),
						'fn_credit' => ascii_to_entities(trim(number_format($xRow->c, 2)))
					);
			}
		}
	...
	function jurnaltotal()
	{
		$sRefno = trim($this->input->post('fs_refno'));

		$this->load->model('mJual','',true);
		$sSQL = $this->mJual->total($sRefno);
		echo '({"sukses": true,"fn_total_debit":'.$sSQL->d.',"fn_total_credit":'.$sSQL->c.'})';
	}
	...
	function jual_ceksave()
	{
		...
		{
			$this->load->model('mJual','',true);
			$ssql = $this->mJual->cek_rangka($rangka);
			if ($ssql->num_rows() > 0)
			{
				$ssql = $ssql->row();
				...
				$hasil = array(
					'sukses'	=> false,
					'hasil'		=> 'Saving Failed,</br>Chasis already exists and already sold with reference: '.trim($refnojual).' !!'
				);
				echo json_encode($hasil);
			}
			else
			{
				$this->load->model('mJual','',true);
				$ssql = $this->mJual->cek_mesin($mesin);
				if ($ssql->num_rows() > 0)
				{
					$ssql = $ssql->row();
					$refnojual = $ssql->fs_refnoinv;
					$hasil = array(
						'sukses'	=> false,
						'hasil'		=> 'Saving Failed,</br>Machine already exists and already sold with reference: '.trim($refnojual).' !!'
					);
					echo json_encode($hasil);
				}
				else
	...
	function jual_save()
	{
		...
		$refnodp = trim($this->input->post('fs_refno_dp'));
		...
		$ketcash = trim($this->input->post('fs_ket_cash'));
		...
		$this->load->model('mJual','',true);
		$ssql = $this->mJual->cek_hargabeli($refnodt,$kdprod);
		...
		$this->load->model('mJual','',true);
		$ssql = $this->mJual->cek_hargabeli2($refnodt,$kdprod);
		...
		$this->load->model('mJual','',true);
		$ssql = $this->mJual->disc_max($refno,$kdprod);
		...
		$where = "fs_kd_comp = '".trim($this->session->userdata('gComp'))."'
					...
					AND	fs_count = '".trim($this->session->userdata('gCount'))."'
					AND	fs_kd_trx = '5300'
					AND	fs_refno = '".trim($refnodp)."'";
		
		$this->db->where($where);
		$this->db->delete('tx_cbdetail');
		//eof hapus tx_cbdetail
		...
		$this->load->model('mJual','',true);
		$ssql = $this->mJual->pos_jual2($refno,$kdtrx,$kdsalesmtd);
		...
		if (trim($nilaidp) <> 0)
		{
			$data = array(
				...
				'fd_refno'			=> trim($refnodt),
				'fs_refno'			=> trim($refnodp),
				'fs_seqno'			=> '000001',
				'fn_trxamtt'		=> trim($nilaidp),
				'fs_kd_refnot'		=> trim($refno),
			...
			$this->mCBReceive->update_trs2('5300','0100','7',$periode,$refnodp,$refno);
		}
		//eof simpan dp
		...
		if (trim($kdpay) == '0')
		{
			...
			$refnocash = '';
			$this->load->model('mCbReceive','',true);
			$ssql = $this->mCbReceive->cek_kodecash($refno);
			
			if ($ssql->num_rows() > 0)
			{
				$ssql = $ssql->row();
				$refnocash = $ssql->fs_refno;
				$xxupdate = true;
			}
			else
			{
				//generate refno
				$this->load->model('mMainModul','',true);
				$ssql = $this->mMainModul->get_refno($this->session->userdata('gDept'),$this->session->userdata('gCount'),$kdtrx,$kdstrx,$refnodt);
				//eof generate refno
				
				if ($ssql->num_rows() > 0)
				{
					$ssql = $ssql->row();
					$refnocash = $ssql->REFNO;// -> case sensitif <jgn diubah>
				}
				else
				{
					$refnocash = '';
			...
			$dt = array(
				...
				'fs_refno'			=> trim($refnocash),
				'fd_refno'			=> trim($refnodt),
				'fs_docno'			=> trim($nmcust),
				'fd_docno'			=> trim($refnodt),
				'fs_acno'			=> trim($kdacno),
				
				'fd_periode'		=> trim($periode),
				'fn_amount'			=> trim($total),
				'fs_note'			=> trim($ketcash),
				'fb_delete'			=> '0',
			...
			$this->load->model('mCBReceive','',true);
			$this->mCBReceive->update_trs($kdtrx,$kdstrx,$kdtrsta,$periode,$refnocash);
			...
			$this->load->model('mCBReceive','',true);
			$this->mCBReceive->update_trs2('5000','0100','7',$periode,$refnocash,$refno);
			//eof simpan detil
		}
		//eof simpan cash*/
		
		$result = $this->mJual->data_refno($refno);
		if ($xupdate == false)
		{
			$hasil = array(
				'sukses'	=> true,
				'hasil'		=> 'Saving Sales Success',
				'refno'		=> $refno,
				'data'		=> $result
			);
			echo json_encode($hasil);
	...
	function jual_cekremove()
	{
		...
		{
			$this->load->model('mJual','',true);
			$ssql = $this->mJual->cek_refno('JL',$refno);
	...
	function jual_remove()
	{
		...
		$this->load->model('mJual','',true);
		$ssql = $this->mJual->reset_reg($refno);
		//eof hapus jual
		
		$hasil = array(
			'sukses'	=> true,
			'hasil'		=> 'Remove sales reference number: "'.$refno.'" success'
		);
		echo json_encode($hasil);
	}
	...
	function printdo()
	{
		...
		$oExcel->getActiveSheet()->getPageSetup()->setPaperSize(PHPExcel_Worksheet_PageSetup::PAPERSIZE_A4);
		$oExcel->getActiveSheet()->getPageMargins()->setTop(0);
		$oExcel->getActiveSheet()->getPageMargins()->setBottom(0);
		$oExcel->getActiveSheet()->getPageMargins()->setLeft(0.2);
		$oExcel->getActiveSheet()->getPageMargins()->setRight(0);
		// $oExcel->getActiveSheet()->getHeaderFooter()->setOddFooter('&L&B' . '&RPage &P of &N');
		$oExcel->getDefaultStyle()->getFont()->setName('Calibri');
		...
		if ($ssql->num_rows() > 0)
		{
			...
			$PaymentName = $ssql->fs_nm_payment;
			$CustomerName = $ssql->fs_nm_cussup;
			$strCustomerName = substr($CustomerName, 0, strrpos($CustomerName, ' '));
			$CustomerAddr = $ssql->fs_addr;
			$TypeName = $ssql->fs_nm_product;
			$Color = $ssql->fs_nm_warna;
			$Year = $ssql->fs_thn;
			...
			for($x = 1; $x <= 3; $x++) {
				...
				$i++;
				$k++;
				$sel->setCellValue('A'.$i,'[  ] Kunci Kontak          [  ] Kaca Spion          [  ] Toolkit          [  ] Buku Servis & Manual          [  ] Jaket           [  ] Helm          [  ] ...');
				$sel->mergeCells('A'.$i.':F'.$i);
				$i = $i + 2;
				$k = $k + 2;
				...
				$sel->setCellValue('A'.$i,$strCustomerName);
				$sel->getStyle('A'.$i)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
				$sel->getStyle('A'.$i)->getAlignment()->setWrapText(true);
				$j = $i;
				$sel->mergeCells('A'.$i.':B'.$j);
	...
	function printkwitansidp()
	{
		...
		$this->load->model('mSearch','',true);
		$ssql = $this->mSearch->cb_trs_all($trx,$strx,$cust,$refno,'0',$cari);
	...
	function printfidusia()
	{
		...
		$docno = trim($this->input->post('fs_refno'));
		$cust = trim($this->input->post('fs_refno'));
		$cari = trim($this->input->post('fs_refno'));
	...
	function printsrtpernyataan()
	{
		...
		if ($ssql->num_rows() > 0)
		{
			...
			$this->load->model('mJual','',true);
			$this->mJual->update_alias($ChassisNumber,$MachineNumber,$sNamaSTNK);
